...
class SesKaydiUygulamasi:
    ...
    def recognize_gender(self):
        ...
        if __name__ == "__main__":
            ...
            olasilik = self.encrypt_data(f"{male_prob * 100:.2f}%" if gender == "Erkek" else f"{female_prob * 100:.2f}%")
            gender2 = self.encrypt_data(gender)
            isim = self.encrypt_data(self.isim_entry.get())
            soyisim = self.encrypt_data(self.soyisim_entry.get())
            tarih_saat = self.encrypt_data(datetime.now().strftime("%d-%m-%Y_%H-%M-%S"))
            Dosya_Adi = self.encrypt_data(f"{isim}_{soyisim}_{tarih_saat}.wav")
            Dosya_Yolu= self.encrypt_data(os.path.join('C:\\Users\\murat\\PycharmProjects\\SesFinal', f"{isim}_{soyisim}_{tarih_saat}.wav"))


            self.insert_record(isim, soyisim, tarih_saat, gender2, olasilik, Dosya_Adi, Dosya_Yolu)
            _, snd_data = record()
